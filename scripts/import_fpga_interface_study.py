...
def main():
 ...
 for rel in ['release_audit/mezzanine/Mezzanine_Fit.png','release_audit/mezzanine/Mezzanine_Fit.svg','release_audit/mezzanine/Mezzanine_Contact_Proposal.csv','release_audit/mezzanine/Mezzanine_Proposal.json','release_audit/mezzanine/Independent_Mezzanine_Review.md','release_audit/mezzanine/Independent_Mezzanine_Review.json','release_audit/mezzanine/Mezzanine_Fit_DRC.json','release_audit/Full_System_Interface_Proposal.md','release_audit/Power_Electrical_Release_Audit.md','reports/AC_IN_IMP_TST_Primary_Source_Review.md','reports/AC_IN_IMP_TST_Reference_Endpoints.json']:
  ...
  if src.suffix=='.md':
   ...
   content=text.encode()
  out.write_bytes(content);records.append({'path':outrel,'sha256':sha(out),'source_sha256':sourcehash,'bytes':len(content),'transformation':'portable reference links only' if sha(out)!=sourcehash else 'unchanged copy'})
