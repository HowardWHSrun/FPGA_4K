...
The `Howard-Wang` branch contains the [FPGA working draft](https://github.com/HowardWHSrun/FPGA_4K/blob/Howard-Wang/hardware/fpga-board/README.md), imported at the user's request for team development. Its [baseline and snapshot evidence](https://github.com/HowardWHSrun/FPGA_4K/blob/Howard-Wang/sources/fpga-draft-2026-09-21/README.md) distinguish the editable design from historical sources. Native CAD and local libraries were copied unchanged; Git tracks later edits. On that branch, the immutable manifest covers the import record and selected evidence, not the evolving CAD files. The current `main` tree excludes Howard’s personal draft and its snapshot evidence; earlier published commits retain their history. Private chat/audio, unrelated personal material, experimental generators, rejected candidates, caches and hidden histories remain outside the shared package.
