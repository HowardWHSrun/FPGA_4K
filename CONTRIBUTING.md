...
Start with [team setup](docs/team/README.md), [owners and open work](docs/team/owners-and-work.md), and the [current 100T review](presentation/fpga/index.html).
...
4. For the most-routed current core, open `hardware/fpga-100t-review/hardware/FPGA100T_Minimal.kicad_pro`. The rail revision and mezzanine fit are separate downloads on the review page. Keep the complete project folder and its libraries. These are review snapshots, not one integrated design.
...
2. Save and close KiCad. In GitHub Desktop, select the agreed development baseline, **Fetch origin**, then **Pull origin** (`presentation` holds the current 100T review snapshots; do not assume the older `main` import is current). Commit unfinished work on its branch before changing branches; do not discard it.
3. Create a short-lived branch, for example `fpga/jtag-link` or `docs/connector-decisions`.
...
A pushed branch is shared work in progress. `presentation` contains the current 100T review snapshots; the integration baseline must be explicitly named in the layout task. Neither branch name implies a fabrication release. A commit stays local until pushed, and teammates receive merged updates only after pulling.
...
kicad-cli sch erc --format json --output build/review/erc.json hardware/fpga-100t-review/hardware/FPGA100T_Minimal.kicad_sch
kicad-cli pcb drc --schematic-parity --format json --output build/review/drc.json hardware/fpga-100t-review/hardware/FPGA100T_Minimal.kicad_pcb
```
...
The `hardware/fpga-board/` September 21 import is historical. The current core, rail revision and fit study are review snapshots, not an integrated development baseline. Before integration, record the chosen revision and copy its complete project into a named development folder with provenance. Preserve original references and frozen reports; regenerate new evidence after CAD changes.
